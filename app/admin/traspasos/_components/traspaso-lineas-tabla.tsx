...
const cellPad = "border-b border-sky-500/20 border-r border-sky-500/30 px-2 py-2 align-top text-xs";
const inp =
  "w-full min-w-0 rounded border border-white/10 bg-slate-950/90 px-1.5 py-1 text-[11px] font-mono text-slate-100 outline-none focus:border-sky-500/50";

const N_COLS = 10;

export function TraspasoLineasTabla({
  lineas,
  onCantidadChange,
  onCantidadBlur,
  onRemove,
}: {
  lineas: TraspasoLineaRow[];
  onCantidadChange: (key: string, value: string) => void;
  onCantidadBlur?: (key: string) => void;
  onRemove: (key: string) => void;
}) {
  return (
    <div className="overflow-x-auto rounded-2xl border border-sky-500/25 bg-slate-900/40">
      <table className="w-full min-w-[960px] border-collapse border border-sky-500/30 text-left text-xs">
        <thead className="border-b-2 border-sky-500/40 bg-slate-950/95 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
          <tr>
            ...
            <th className={`${cellPad} w-20 text-right whitespace-nowrap`}>Stock origen</th>
            <th className={`${cellPad} w-24 text-right whitespace-nowrap`}>Cantidad</th>
          ...
          {lineas.length === 0 ? (
            <tr>
              <td colSpan={N_COLS} className="px-4 py-10 text-center text-sm text-slate-500">
                Todavía no hay productos en este traspaso. Buscá en el catálogo de arriba y pulsá Agregar.
              </td>
            </tr>
          ) : (
            lineas.map((ln) => {
              ...
              const restante = ln.stockOrigen - cantOk;
              const restNeg = restante < 0;
              const img0 = ln.imagenesUrls[0] ?? "";
              return (
                <tr key={ln.key} className="bg-slate-950/25 hover:bg-sky-950/20">
                  <td className={`${cellPad} text-center`}>
                    <div className="flex flex-col items-center gap-1">
                      <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded border border-sky-500/25 bg-black/40">
                        {img0 ? (
                  ...
                  <td className={`${cellPad} font-mono text-sky-200`}>{ln.codigo}</td>
                  ...
                  <td className={`${cellPad} text-right font-mono text-emerald-300`}>{ln.stockOrigen}</td>
