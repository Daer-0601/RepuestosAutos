...
export async function POST(request: Request) {
  ...
  if (!Number.isFinite(sucursalOrigenId) || sucursalOrigenId < 1) {
    return NextResponse.json({ error: "Elegí sucursal origen." }, { status: 400 });
  }

  const perRaw = b.perPage !== undefined ? Number(b.perPage) : CATALOGO_FILAS_DEFAULT;
  const pageSize = Number.isFinite(perRaw) && perRaw >= 10 ? sqlInt(Math.trunc(perRaw), CATALOGO_FILAS_MAX) : CATALOGO_FILAS_DEFAULT;
  ...
  if (b.soloConStockEnOrigen === true) {
    filtros.sucursalStockId = sucursalOrigenId;
  }
