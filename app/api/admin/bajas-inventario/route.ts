...
import {
  registrarBajaInventario,
  type BajaInventarioLineaInput,
} from "@/lib/data/bajas-inventario";
import { searchProductosConStockEnSucursal } from "@/lib/data/traspasos";
import { revalidatePath } from "next/cache";
import { NextResponse } from "next/server";

function parseLinea(raw: unknown): BajaInventarioLineaInput | null {
...
export async function POST(request: Request) {
  ...
  const lineas: BajaInventarioLineaInput[] = [];
  ...
  const res = await registrarBajaInventario({
  ...
  revalidatePath("/admin/bajas-inventario");
